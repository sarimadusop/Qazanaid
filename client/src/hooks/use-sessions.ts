...
import { type InsertOpnameSession, type OpnameSession } from "@shared/schema";
...
export function useCreateSession() {
  ...
  return useMutation({
    mutationFn: async (data: InsertOpnameSession) => {
...
export function useCompleteSession() {
  ...
  return useMutation({
    mutationFn: async (id: number) => {
      ...
      const res = await fetch(url, { method: api.sessions.complete.method });
...
export function useUpdateRecord() {
  ...
  return useMutation({
    ...
    onSuccess: (_, { sessionId }) => {
      // Optimistic update logic is complex, simpler to invalidate for now
